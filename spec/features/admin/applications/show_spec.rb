...
RSpec.describe 'the /admin/applications/:id show page', type: :feature do
  ...
  let!(:application_pets_6) { ApplicationPet.create!(application_id: application_3.id, pet_id: pet_1.id) }
  ...
  describe 'rejecting a pet for adoption' do
    ...
    it 'returns to the /admin/applications/:id show page and the approve button AND reject button is gone' do
      visit "/admin/applications/#{application_2.id}"      
  ...
  describe 'multiple approved pets on different applications' do
    ...
    it 'allows a pet can be declined on one application and approved on another' do
      ...
      expect(page).to_not have_button("Reject Lucky")
      expect(page).to have_content("Lucky Rejected!")
